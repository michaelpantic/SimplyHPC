...
Figure \ref{fig:schemaService} visualizes all relevant components of a compute cluster that has been set up using our framework. 	
\begin{itemize}
	\item A \textit{cloud service} is a bundle of multiple nodes using the same deployment template. Using the logical entity of a cloud service, nodes can be configure (number, size, network) at once. E.g. increasing the number of nodes can be done by increasing the number in the cloud service configuration. The azure cloud controller then automatically creates a new node and applies the deployment template.

	\item Interconnect: When using A8/A9 nodes for scientific computing, the internode network (the upper arrow in figure \ref{fig:schemaService}) is realized by infiniband. All internode storage (SMBv3) and MPI (MSMPIv2) requests are transparently redirected to this very low latency, high bandwidth interconnect. This is crucial for good scaling and performance.

	\item Storage: There are 4 storage types in use: Local storage, which uses local SSD drives and can be shared between nodes. Azure Table and Blob Storage, which is accessible using HTTP/REST interfaces and Azure Virtual Drives, which can be mounted using SMB. Of these 4 types, only the local storage can be accessed transparently using infiniband between the nodes. All other types are accessed using internet access and/or internal Azure networks, and are therefore much less performant.
\end{itemize}
\begin{figure}[ht]
	\centering
